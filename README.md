...
| Arduino       | Компонент       | Назначение    |
|---------------|-----------------|---------------|
| Pin 2 + GND   | Кнопка (BTN 1)  | Диагностика   |
| Pin 3 + GND   | Кнопка (BTN 2)  | Восстановление|

### Правила подключения
- Светодиод: **длинная ножка (+)** → резистор 220 Ом → пин Arduino
- Светодиод: **короткая ножка (−)** → GND
- Кнопка: одна ножка → пин, вторая → GND  
  (внешний резистор не нужен — используется `INPUT_PULLUP`)
