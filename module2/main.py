...
DIRS=[(1,0),(-1,0),(0,1),(0,-1)]; CELL,COLS,ROWS=36,21,21
...
ROUTES={}; TICK=0; TL_STATS={}; CAR_STATS={}

# ── DB ───────────────────────────────────────────────────────────────────────
def db(sql,args=()): con=sqlite3.connect(DB_FILE); con.execute(sql,args); con.commit(); con.close()
...
def db_spawn(car_id): CAR_STATS[car_id]=TICK
def db_exit(car_id):
    if car_id in CAR_STATS: db("INSERT INTO Cars_stats VALUES(?,?,?)",(car_id,CAR_STATS.pop(car_id),TICK))
def db_pass_tl(tid): TL_STATS.setdefault(tid,{"n":0,"sw":0,"t":TICK})["n"]+=1
def db_sw_tl(tid):   TL_STATS.setdefault(tid,{"n":0,"sw":0,"t":TICK})["sw"]+=1
def db_flush():
...
def tl_near(objs,c):
    return any(objs.get((c[0]+dx,c[1]+dy),{}).get("path") in TL_STOP|BLOCK for dx,dy in DIRS if (c[0]+dx,c[1]+dy)!=c)
def step(cars,objs,anim,routes):
    ...
    for k,st in anim.items():
        ...
        if car:
            ...
            if objs.get(nxt,{}).get("base")==IMG5: db_pass_tl(f"{nxt[0]},{nxt[1]}")
...
class Grid(QWidget):
    ...
    def load(self,path):
        data=load_j(path)
        self.roads=parse(data.get("roads",{})); self.objs=parse(data.get("objs",{}))
...
class App(QMainWindow):
    def __init__(self):
        ...
        for name,chk,on,off in[
            ("Диагностика",False,lambda:None,lambda:None),("Восстановление",False,lambda:None,lambda:None),
    ...
    def _tl_tick(self):
        ...
        for c,o in self.grid.objs.items():
            if o["base"]==IMG5: o["path"]=state; db_sw_tl(f"{c[0]},{c[1]}")
