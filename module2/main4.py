...
try:
self._s = serial.Serial(port, 9600, timeout=0.1)
time.sleep(2)  # ждём перезагрузку Arduino после открытия порта
except: pass
def tl(self, path):
name = os.path.basename(path).lower()
cmd = “G” if “green” in name else (“R” if “red” in name else “Y”)
try:
if self._s:
